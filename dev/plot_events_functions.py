...
def get_nc_files(vars):
    ...
    if 'latitude' in mhw.coords: mhw = mhw.rename({'latitude': 'lat'})
    if 'longitude' in mhw.coords: mhw = mhw.rename({'longitude': 'lon'})
...
def load_and_filter_nc_files(file_paths, start_date, end_date):
    ...
    for file_path in file_paths:
        ...
        if not filtered_ds.time.size == 0:
            ...
            filtered_ds['event'] = filtered_ds['event'] * lbl
